...
class MYSQLConnector:
    ...
    def loader(self, limit):
        query = db.select([self.Table]).limit(limit)
...
class Sales_MYSQLConnector(MYSQLConnector):
    ...
    def filter_Time(self, date_range=dict, hour=False, date=False, dayOfWeek=False, month=False,
                    quarter=False):
        """

        :param date_range: {'start': 'x', 'end': 'x'} 형태로 들어와야 함
        :param hour:
        :param date:
        :param dayOfWeek:
        :param month:
...
if __name__ == '__main__':
    try:
        # print(sys.argv)
        x = json.loads(sys.argv[1].replace("\'", "\""))
        #print(x)
        #print(type(x))
        ...
        """
        실행방법 
        1. Sales_MYSQLConnector 클래스 가져오기
        2. loader함수 실행 (데이터 갯수 제한 지정:limit)
        3. select_Sales(), select_Visitors(), select_Volumes_Products(), select_Volumes_Orders() 실행 - 마지막에  실행된 하나의 
        함수만 적용됨
        
        4. filter_Time, filter_Department, filter_Customers 실행 : 중복 적용 가능
        
        
        (1) filter_Time: hour, date, dayOfWeek, month, quarter : Boolean 형태로 입력
            date_range: {'start': datetime, 'end': datetime} 형태로 들어와야 함
        ...
        (3) filter_Customers:
        {'id': []} > 고객id 항목을 dictionary 안의 list 형태로 받는다. 각 항목은 이름(string)이 아닌 id값을 가지고 있어야 함.
                    SELECT ALL을 선택한 경우 [-1]값을 받는다.
                    
        5. filter_execute() 함수 실행 > 결과 테이블을 return
        """
        ...
        # """
        sql_connector.loader(limit=100000)
        ...
        # sql_connector.select_Visitors()
        # sql_connector.select_Volumes_Products()
        # sql_connector.select_Volumes_Orders()


        # sql_connector.filter_Time(hour=True)
        sql_connector.fileter_Department({'dept':[-1]})
        result = pd.DataFrame(sql_connector.filter_execute()).to_json()
        print(result)
